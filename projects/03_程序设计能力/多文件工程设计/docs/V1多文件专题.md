...
`A.h` 的 include guard 避免同一翻译单元重复包含。原文件同时在头文件里定义 `age`，而 `main.c` 并未使用它；多个 `.c` 同时包含该头文件时可能产生重复定义。课程示例保留原写法，文档记录该接口边界。
...
这个例子展示“声明供调用方使用，实现参与最终链接”。头文件既是接口入口，也可能引入依赖和全局定义风险；[counter 版本](../practice/counter.h)把状态留在实现文件，与课程示例形成对照。
